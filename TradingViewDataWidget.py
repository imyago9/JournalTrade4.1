...
from functions import TV_ACCOUNTS_DATA_PATH, TV_ACCOUNT_SETTINGS_PATH, TV_SCREENSHOTS_PATH
import os
import sys
...
from SettingsDataWidget import SettingsWidget
...
class TradingViewDataWidget(QDialog):
    def __init__(self, parent=None):
        super(TradingViewDataWidget, self).__init__(parent)
        ...
        self.mapping_file = os.path.join(TV_SCREENSHOTS_PATH, 'screenshot_mapping.json')
    ...
    def setupDataManagementFrame(self):
        ...
        account_settings_icon.addPixmap(QtGui.QPixmap(resource_path("resources/settings-v-svgrepo-com.svg")), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def setupTopFrameTitle(self):
        self.top_frame_title = QLabel("TradingView Account(s) Data")
        font = QtGui.QFont()
    ...
    def load_accounts(self):
        try:
            self.accounts = [f.split('.')[0] for f in os.listdir(TV_ACCOUNTS_DATA_PATH) if f.endswith('.csv')]
            self.accountselection_comboBox.clear()
            self.accountselection_comboBox.addItems(self.accounts)
    ...
    def load_account_data(self, account):
        try:
            file_path = os.path.join(TV_ACCOUNTS_DATA_PATH, f'{account}.csv')
    ...
    def display_selected_account(self):
        try:
            ...
            self.account_data = self.load_account_data(self.selected_account)
    ...
    def updateDateEdits(self):
        ...
        account_first_date = pd.to_datetime(self.account_data['Time']).min()
        account_last_date = pd.to_datetime(self.account_data['Time']).max()
    ...
    def updateStatsLabels(self, data):
        grouped_data = data.groupby('Time').agg({'Profit': 'sum', 'LorS': 'first', 'Instrument': 'first'}).reset_index()
        net_pnl = grouped_data['Profit'].sum()
        total_trades = len(grouped_data)
        winning_trades = len(grouped_data[grouped_data['Profit'] > 0])
    ...
    def plot_line_graph(self, data):
        try:
            ...
            cumulative_progress = data.groupby('Time')['Profit'].sum().cumsum()
            fig, ax = plt.subplots()
    ...
    def plot_trade_performance(self, data):
        try:
            ...
            if filtered_data.empty:
                ...
                return
            grouped_data = filtered_data.groupby('Time').agg(Profit=('Profit', 'sum'), Qty=('Qty', 'sum'), LorS=('LorS', 'first')).reset_index()
    ...
    def annotate_bar(self, sel, grouped_data):
        sel.annotation.set_text(
            f"Date: {pd.to_datetime(grouped_data.iloc[sel.index]['Time']).strftime('%m/%d - %H:%M')}\n" +
    ...
    def on_click(self, event, bars, grouped_data):
        for i, bar in enumerate(bars):
            if bar.contains(event)[0]:
                ...
                trade_id = f"{self.selected_account}_{selected_data['Time']}"
                ...
                if sanitized_trade_id in self.screenshots:
                    ...
                    self.prompt_add_screenshot(sanitized_trade_id, selected_data, selected_account=self.selected_account)
                break

    def prompt_add_screenshot(self, trade_id, trade_info, selected_account):
        dialog = ScreenshotPrompt(trade_info, account_type='TradingView', selected_account=selected_account)
        if dialog.exec_() == QDialog.Accepted:
            upload_dialog = ScreenshotUpload(trade_info, trade_id, TV_SCREENSHOTS_PATH, account_type='TradingView', selected_account=selected_account)
    ...
    def plot_calendar(self, data):
        try:
            ...
            self.calendar = CustomCalendar(parent=None, account_type='TradingView')
    ...
    def update_calendar(self, data):
        data = data.groupby('Time').agg({'Profit': 'sum', 'LorS': 'first'}).reset_index()
        data['Time'] = pd.to_datetime(data['Time'])
        data['Date'] = data['Time'].dt.date
        grouped_data = data.groupby('Date').agg({'Profit': 'sum', 'Time': 'count'}).rename(columns={'Time': 'Trades'}).reset_index()
    ...
    def open_settings_widget(self):
        if not self.accountselection_comboBox.currentIndex() == -1:
            self.settings_widget = SettingsWidget(selected_account=self.selected_account, data_path=TV_ACCOUNT_SETTINGS_PATH, account_type='TradingView')
            self.settings_widget.update_values()
            self.settings_widget.closed.connect(self.enable_data_widget)
            self.settings_widget.reloadPressed.connect(self.update_graph)
            self.settings_widget.show()
            self.setEnabled(False)

    def enable_data_widget(self):
