...
from functions import NT_TRADE_DATA_PATH, NT_ACCOUNT_SETTINGS_PATH, NT_SCREENSHOTS_PATH
import os
import json
import re
import sys
...
class NinjaTraderDataWidget(QDialog):
    def __init__(self, parent=None):
        super(NinjaTraderDataWidget, self).__init__(parent)
        ...
        self.mapping_file = os.path.join(NT_SCREENSHOTS_PATH, 'screenshot_mapping.json')
    ...
    def setupTopFrameTitle(self):
        self.top_frame_title = QLabel("NinjaTrader Account(s) Data")
        font = QtGui.QFont()
    ...
    def load_accounts(self):
        try:
            self.data = pd.read_csv(NT_TRADE_DATA_PATH)
            self.data_settings = pd.read_csv(NT_ACCOUNT_SETTINGS_PATH)
            visible_accounts = self.data_settings[self.data_settings['Visibility'] == 'visible']['Account']
            self.accountselection_comboBox.clear()
            self.accountselection_comboBox.addItems(visible_accounts)
    ...
    def on_click(self, event, bars, grouped_data):
        for i, bar in enumerate(bars):
            if bar.contains(event)[0]:
                ...
                selected_account = self.accountselection_comboBox.currentText()
                if sanitized_trade_id in self.screenshots:
                    self.show_screenshot(self.screenshots[sanitized_trade_id])
                else:
                    self.prompt_add_screenshot(sanitized_trade_id, selected_data, selected_account=selected_account)
                break


    def prompt_add_screenshot(self, trade_id, trade_info, selected_account):
        dialog = ScreenshotPrompt(trade_info, selected_account=selected_account)
        if dialog.exec_() == QDialog.Accepted:
            upload_dialog = ScreenshotUpload(trade_info, trade_id, NT_SCREENSHOTS_PATH, selected_account=selected_account)
    ...
    def plot_calendar(self, data):
        try:
            ...
            self.calendar = CustomCalendar(parent=None, account_type='NinjaTrader')
    ...
    def open_settings_widget(self):
        if not self.accountselection_comboBox.currentIndex() == -1:
            self.settings_widget = SettingsWidget(selected_account=self.selected_account, data_path=self.data_settings)
